...
function run() {
    ...
    echo "Start Command: nohup java -jar $appDir/$appName --logback.loglevel=DEBUG --spring.profiles.active=$profiles --server.port=$serverPort >> $appStartLog 2>&1 &\n"
    . /etc/profile
    nohup java -jar $appDir/$appName --logback.loglevel=DEBUG --spring.profiles.active=$profiles --server.port=$serverPort >> $appStartLog 2>&1 &
}
