...
export function AppSidebarNavGroup({ title, items }: AppSidebarNavGroup) {
...
function checkIsActive(href: string, item: NavItem, mainNav = false) {
  ...
  if (hrefWithoutQuery === item.url) {
    return true
  }

  // Match child item
  if (item.items && item.items.some(i => i.url === href)) {
    return true
  }

  // Match main navigation group by first path segment
  if (mainNav) {
    const firstSegment = href.split("/")[1]
    const itemSegment = item.url?.split("/")[1]
    if (firstSegment && firstSegment === itemSegment) {
      return true
    }
  }
