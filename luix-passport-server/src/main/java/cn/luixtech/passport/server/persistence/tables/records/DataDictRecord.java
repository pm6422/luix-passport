/*
 * This file is generated by jOOQ.
 */
package cn.luixtech.passport.server.persistence.tables.records;


import cn.luixtech.passport.server.persistence.tables.DataDict;

import java.time.Instant;

import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record10;
import org.jooq.Row10;
import org.jooq.impl.UpdatableRecordImpl;


/**
 * This class is generated by jOOQ.
 */
@SuppressWarnings({ "all", "unchecked", "rawtypes", "this-escape" })
public class DataDictRecord extends UpdatableRecordImpl<DataDictRecord> implements Record10<String, String, String, String, String, Boolean, Instant, String, Instant, String> {

    private static final long serialVersionUID = 1L;

    /**
     * Setter for <code>public.data_dict.id</code>.
     */
    public void setId(String value) {
        set(0, value);
    }

    /**
     * Getter for <code>public.data_dict.id</code>.
     */
    public String getId() {
        return (String) get(0);
    }

    /**
     * Setter for <code>public.data_dict.category_code</code>.
     */
    public void setCategoryCode(String value) {
        set(1, value);
    }

    /**
     * Getter for <code>public.data_dict.category_code</code>.
     */
    public String getCategoryCode() {
        return (String) get(1);
    }

    /**
     * Setter for <code>public.data_dict.dict_code</code>.
     */
    public void setDictCode(String value) {
        set(2, value);
    }

    /**
     * Getter for <code>public.data_dict.dict_code</code>.
     */
    public String getDictCode() {
        return (String) get(2);
    }

    /**
     * Setter for <code>public.data_dict.dict_name</code>.
     */
    public void setDictName(String value) {
        set(3, value);
    }

    /**
     * Getter for <code>public.data_dict.dict_name</code>.
     */
    public String getDictName() {
        return (String) get(3);
    }

    /**
     * Setter for <code>public.data_dict.remark</code>.
     */
    public void setRemark(String value) {
        set(4, value);
    }

    /**
     * Getter for <code>public.data_dict.remark</code>.
     */
    public String getRemark() {
        return (String) get(4);
    }

    /**
     * Setter for <code>public.data_dict.enabled</code>.
     */
    public void setEnabled(Boolean value) {
        set(5, value);
    }

    /**
     * Getter for <code>public.data_dict.enabled</code>.
     */
    public Boolean getEnabled() {
        return (Boolean) get(5);
    }

    /**
     * Setter for <code>public.data_dict.created_at</code>.
     */
    public void setCreatedAt(Instant value) {
        set(6, value);
    }

    /**
     * Getter for <code>public.data_dict.created_at</code>.
     */
    public Instant getCreatedAt() {
        return (Instant) get(6);
    }

    /**
     * Setter for <code>public.data_dict.created_by</code>.
     */
    public void setCreatedBy(String value) {
        set(7, value);
    }

    /**
     * Getter for <code>public.data_dict.created_by</code>.
     */
    public String getCreatedBy() {
        return (String) get(7);
    }

    /**
     * Setter for <code>public.data_dict.modified_at</code>.
     */
    public void setModifiedAt(Instant value) {
        set(8, value);
    }

    /**
     * Getter for <code>public.data_dict.modified_at</code>.
     */
    public Instant getModifiedAt() {
        return (Instant) get(8);
    }

    /**
     * Setter for <code>public.data_dict.modified_by</code>.
     */
    public void setModifiedBy(String value) {
        set(9, value);
    }

    /**
     * Getter for <code>public.data_dict.modified_by</code>.
     */
    public String getModifiedBy() {
        return (String) get(9);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    @Override
    public Record1<String> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record10 type implementation
    // -------------------------------------------------------------------------

    @Override
    public Row10<String, String, String, String, String, Boolean, Instant, String, Instant, String> fieldsRow() {
        return (Row10) super.fieldsRow();
    }

    @Override
    public Row10<String, String, String, String, String, Boolean, Instant, String, Instant, String> valuesRow() {
        return (Row10) super.valuesRow();
    }

    @Override
    public Field<String> field1() {
        return DataDict.DATA_DICT.ID;
    }

    @Override
    public Field<String> field2() {
        return DataDict.DATA_DICT.CATEGORY_CODE;
    }

    @Override
    public Field<String> field3() {
        return DataDict.DATA_DICT.DICT_CODE;
    }

    @Override
    public Field<String> field4() {
        return DataDict.DATA_DICT.DICT_NAME;
    }

    @Override
    public Field<String> field5() {
        return DataDict.DATA_DICT.REMARK;
    }

    @Override
    public Field<Boolean> field6() {
        return DataDict.DATA_DICT.ENABLED;
    }

    @Override
    public Field<Instant> field7() {
        return DataDict.DATA_DICT.CREATED_AT;
    }

    @Override
    public Field<String> field8() {
        return DataDict.DATA_DICT.CREATED_BY;
    }

    @Override
    public Field<Instant> field9() {
        return DataDict.DATA_DICT.MODIFIED_AT;
    }

    @Override
    public Field<String> field10() {
        return DataDict.DATA_DICT.MODIFIED_BY;
    }

    @Override
    public String component1() {
        return getId();
    }

    @Override
    public String component2() {
        return getCategoryCode();
    }

    @Override
    public String component3() {
        return getDictCode();
    }

    @Override
    public String component4() {
        return getDictName();
    }

    @Override
    public String component5() {
        return getRemark();
    }

    @Override
    public Boolean component6() {
        return getEnabled();
    }

    @Override
    public Instant component7() {
        return getCreatedAt();
    }

    @Override
    public String component8() {
        return getCreatedBy();
    }

    @Override
    public Instant component9() {
        return getModifiedAt();
    }

    @Override
    public String component10() {
        return getModifiedBy();
    }

    @Override
    public String value1() {
        return getId();
    }

    @Override
    public String value2() {
        return getCategoryCode();
    }

    @Override
    public String value3() {
        return getDictCode();
    }

    @Override
    public String value4() {
        return getDictName();
    }

    @Override
    public String value5() {
        return getRemark();
    }

    @Override
    public Boolean value6() {
        return getEnabled();
    }

    @Override
    public Instant value7() {
        return getCreatedAt();
    }

    @Override
    public String value8() {
        return getCreatedBy();
    }

    @Override
    public Instant value9() {
        return getModifiedAt();
    }

    @Override
    public String value10() {
        return getModifiedBy();
    }

    @Override
    public DataDictRecord value1(String value) {
        setId(value);
        return this;
    }

    @Override
    public DataDictRecord value2(String value) {
        setCategoryCode(value);
        return this;
    }

    @Override
    public DataDictRecord value3(String value) {
        setDictCode(value);
        return this;
    }

    @Override
    public DataDictRecord value4(String value) {
        setDictName(value);
        return this;
    }

    @Override
    public DataDictRecord value5(String value) {
        setRemark(value);
        return this;
    }

    @Override
    public DataDictRecord value6(Boolean value) {
        setEnabled(value);
        return this;
    }

    @Override
    public DataDictRecord value7(Instant value) {
        setCreatedAt(value);
        return this;
    }

    @Override
    public DataDictRecord value8(String value) {
        setCreatedBy(value);
        return this;
    }

    @Override
    public DataDictRecord value9(Instant value) {
        setModifiedAt(value);
        return this;
    }

    @Override
    public DataDictRecord value10(String value) {
        setModifiedBy(value);
        return this;
    }

    @Override
    public DataDictRecord values(String value1, String value2, String value3, String value4, String value5, Boolean value6, Instant value7, String value8, Instant value9, String value10) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        value5(value5);
        value6(value6);
        value7(value7);
        value8(value8);
        value9(value9);
        value10(value10);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached DataDictRecord
     */
    public DataDictRecord() {
        super(DataDict.DATA_DICT);
    }

    /**
     * Create a detached, initialised DataDictRecord
     */
    public DataDictRecord(String id, String categoryCode, String dictCode, String dictName, String remark, Boolean enabled, Instant createdAt, String createdBy, Instant modifiedAt, String modifiedBy) {
        super(DataDict.DATA_DICT);

        setId(id);
        setCategoryCode(categoryCode);
        setDictCode(dictCode);
        setDictName(dictName);
        setRemark(remark);
        setEnabled(enabled);
        setCreatedAt(createdAt);
        setCreatedBy(createdBy);
        setModifiedAt(modifiedAt);
        setModifiedBy(modifiedBy);
        resetChangedOnNotNull();
    }
}
