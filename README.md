...
## 概述 Overview
本项目以SpringBoot2为基础的标准OAuth2服务端程序，使用本项目可以快速创建公司内部的OAuth2服务器。其他基于Web，手机App客户端可以使用本服务端做统一安全认证。
...
## 应用运行
本项目包含服务器端(passport-server)和客户端(passport-client)两个子模块。由于本项目采用嵌入式内存数据库，不需要依赖外部数据库，因此可以做到不修改一行代码就可以完成应用启动。
