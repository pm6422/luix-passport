...
[[access_app]]
== Application URLs
* Navigate to `http://127.0.0.1:6001` to access authorization server
* Navigate to `http://127.0.0.1:6002` to access resource server
* Navigate to `http://127.0.0.1:6003` to access client
...
In the "Set a redirect URI" sub-section, ensure that the *Authorized redirect URIs* field is set to `http://localhost:6001/login/oauth2/code/google-idp`.
...
When registering the OAuth application, ensure the *Authorization callback URL* is set to `http://localhost:6001/login/oauth2/code/github-idp`.
